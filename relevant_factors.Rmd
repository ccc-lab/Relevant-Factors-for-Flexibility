...
has_cases <- function(x){
  ...
  else if(code == 'Exclusively borderline case-marking'){ return(0)}
  else return(1);
}
...
wals_has_case <- read.csv('autotyp-data-master/data/WALS_cases.csv', na.strings='') %>% rename(WALSCode = ID, NumCases = Name) %>% select(WALSCode, NumCases) %>% mutate(NumCases = has_cases(NumCases)) %>% rename(CaseMarking = NumCases)
...
flexibility_data_out <- flexibility_data %>% select(LID, WALSCode, Glottocode, ISO639.3, LanguageAutotyp, LanguageWALS, Area, SubBranch, WALSOrder, WALSTwoOrder, WordOrderSPVBasicLex, WordOrderSPVLexFlexible, Marking, CaseMarking, VerbPersonMarking) %>% group_by(LID)

flexibility_data_out

write.csv(flexibility_data_out, file="flexibility_data",
          row.names=FALSE)
```
...
autotyp_np_structure <- read.csv('autotyp-data-master/data/NP_Structure.csv', na.strings='') %>%
  select(LID, NPMarking.binned4, NPWordOrder)  %>% filter(!is.na(NPMarking.binned4) & !is.na(NPWordOrder))


bib <- read.csv('autotyp-data-master/data/Bibliography.csv', na.strings='') %>%
  select(LID, Author, Year)
